...
const Search = ({ setSearch, listSearchCountry, search, setLocationInfo }) => {
  const [isOpen, setIsOpen] = useState(false);
  const handlerItemCountry = (e) => {
    setSearch(e.target.textContent);
    listSearchCountry
      .filter((items) => items.country.toLowerCase().includes(e.target.textContent.toLowerCase()))
      .map((items) => setLocationInfo(items));
    setIsOpen(false);
  };
